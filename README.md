...
Clients can suggest specific IP addresses — particularly useful for lighthouse nodes or declarative setups (NixOS, Ansible, etc.):

```bash
# Using CLI flag (takes precedence)
python nacme/client.py --ip 10.100.0.10

# Using environment variable (convenient for systemd units)
...
- Valid + available → assigned as requested
- Already taken → silently falls back to automatic allocation
- Invalid (wrong subnet, network/broadcast address, malformed) → 422 error from server
- Client prints "Suggesting IP: …" when a suggestion is sent

**Common use cases:**

- Stable lighthouse IPs (10.100.0.1, 10.100.0.2, …)
- Preserving known IPs during migration to NACME
- Declarative configuration where IP should be known before first boot
...
**Latest release: v0.2.0** (2026-01-29)  
Main new feature: optional client-suggested IP addresses with validation + fallback
